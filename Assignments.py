...
def showPdf(subject_fullname):
    global pdf_path
    con = mysql.connect(host="localhost", user="root", password="", database="kaksha")
    cursor = con.cursor(buffered=True)
    cursor.execute("SELECT * FROM subject WHERE subject_fullname='"+str(subject_fullname)+"'")
    currentSubject = cursor.fetchone()
...
def showParticularPdf(assignment_name):
    con = mysql.connect(host="localhost", user="root", password="", database="kaksha")
    cursor = con.cursor(buffered=True)
    cursor.execute("SELECT * FROM assignment WHERE assignment_name='"+str(assignment_name)+"'")
    currentAssignments = cursor.fetchone()

    path = currentAssignments[5]
    os.startfile(path)


def showAssignments(subject_fullname):
    global subject_fullnamea
    subject_fullnamea = subject_fullname
    con = mysql.connect(host="localhost", user="root", password="", database="kaksha")
    cursor = con.cursor(buffered=True)
    cursor.execute("SELECT * FROM subject WHERE subject_fullname='"+str(subject_fullname)+"'")
    currentSubject = cursor.fetchone()

    cursor.execute("SELECT * FROM assignment WHERE subject_id='"+str(currentSubject[0])+"' AND class_id='"+str(student_session[9])+"'")
    currentAssignments = cursor.fetchall()
    global root
    # root.destroy()
    ...
    title.configure(text=subject_fullname)
    ...
    def select():
        showParticularPdf(subjectsListbox.get(subjectsListbox.curselection()))

    subjectsListbox = tk.Listbox(root, font = "Helvetica", width=47, height=7)
    i=1
    for assignment in currentAssignments:
        subjectsListbox.insert(i, assignment[1])
    subjectsListbox.pack(padx=5, pady=250)

    b_select = tk.Button(rightFrame, command=select)
    b_select.place(relx=0.3, rely=0.53, height=34, width=187)
...
def AssignmentsPage(student_id):
    global root
    global student_session
    global pdf_path
    root = tk.Tk()
    ...
    def select():
        showAssignments(subjectsListbox.get(subjectsListbox.curselection()))

    subjectsListbox = tk.Listbox(root, font = "Helvetica", width=47, height=7)
    i=1
    for subject in subject_session:
        subjectsListbox.insert(i, subject[4])
    subjectsListbox.pack(padx=5, pady=250)
